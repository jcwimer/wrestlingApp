...
class FourPoolToSemiGenerationTest < ActionDispatch::IntegrationTest
  ...
  test 'Match generation works' do
    ...
    assert @tournament.matches.count { |m| m.bracket_position == 'Pool' } == 24
  ...
  test 'Loser names set up correctly' do
    ...
    assert @tournament.matches.find { |m| m.bracket_position == 'Semis' && m.bracket_position_number == 1 }.loser2_name == 'Winner Pool 4'
    assert @tournament.matches.find { |m| m.bracket_position == 'Semis' && m.bracket_position_number == 2 }.loser1_name == 'Winner Pool 2'
    assert @tournament.matches.find { |m| m.bracket_position == 'Semis' && m.bracket_position_number == 2 }.loser2_name == 'Winner Pool 3'
    assert @tournament.matches.find { |m| m.bracket_position == 'Conso Semis' && m.bracket_position_number == 1 }.loser1_name == 'Runner Up Pool 1'
    assert @tournament.matches.find { |m| m.bracket_position == 'Conso Semis' && m.bracket_position_number == 1 }.loser2_name == 'Runner Up Pool 4'
    assert @tournament.matches.find { |m| m.bracket_position == 'Conso Semis' && m.bracket_position_number == 2 }.loser1_name == 'Runner Up Pool 2'
    assert @tournament.matches.find { |m| m.bracket_position == 'Conso Semis' && m.bracket_position_number == 2 }.loser2_name == 'Runner Up Pool 3'
  ...
  test 'Placement points are given when moving through bracket' do
    match = @tournament.matches.find { |m| m.bracket_position == 'Conso Semis' }
