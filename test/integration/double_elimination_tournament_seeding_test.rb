...
class EightManDoubleEliminationSixPlacesRunThrough < ActionDispatch::IntegrationTest
  ...
  test 'Wrestlers with seeds should go on certain lines and it should be random for everyone else' do
    ...
    matches.select { |m| m.bracket_position == 'Quarter' }.each do |match|
      assert_not(match.wrestler1.original_seed.nil?)
  ...
  test 'Regenerating matches without changes should keep lines the same' do
    ...
    test_one_bracket_line_original = wrestlers.find { |w| w.name == 'Test1' }.bracket_line
    test_two_bracket_line_original = wrestlers.find { |w| w.name == 'Test2' }.bracket_line
    test_three_bracket_line_original = wrestlers.find { |w| w.name == 'Test3' }.bracket_line
    test_four_bracket_line_original = wrestlers.find { |w| w.name == 'Test4' }.bracket_line
    test_five_bracket_line_original = wrestlers.find { |w| w.name == 'Test5' }.bracket_line
    test_six_bracket_line_original = wrestlers.find { |w| w.name == 'Test6' }.bracket_line
    test_seven_bracket_line_original = wrestlers.find { |w| w.name == 'Test7' }.bracket_line
    test_eight_bracket_line_original = wrestlers.find { |w| w.name == 'Test8' }.bracket_line

    generate_tournament_matches(@tournament.reload)

    wrestlers = @tournament.reload.wrestlers
    test_one_bracket_line_second = wrestlers.find { |w| w.name == 'Test1' }.bracket_line
    test_two_bracket_line_second = wrestlers.find { |w| w.name == 'Test2' }.bracket_line
    test_three_bracket_line_second = wrestlers.find { |w| w.name == 'Test3' }.bracket_line
    test_four_bracket_line_second = wrestlers.find { |w| w.name == 'Test4' }.bracket_line
    test_five_bracket_line_second = wrestlers.find { |w| w.name == 'Test5' }.bracket_line
    test_six_bracket_line_second = wrestlers.find { |w| w.name == 'Test6' }.bracket_line
    test_seven_bracket_line_second = wrestlers.find { |w| w.name == 'Test7' }.bracket_line
    test_eight_bracket_line_second = wrestlers.find { |w| w.name == 'Test8' }.bracket_line
  ...
  test 'Deleting a wrestler and generating produces a BYE for the person who lost their opponent' do
    wrestler_three_first_round = @tournament.reload.matches.find { |m| m.bracket_position == 'Quarter' && m.wrestler1.name == 'Test3' }
    wrestler_three_first_round.wrestler2.destroy
    generate_tournament_matches(@tournament.reload)
    matches = @tournament.reload.matches

    match_with_bye = matches.find { |m| m.loser2_name == 'BYE' }
  ...
  test 'Deleting a seeded wrestler reseeding and generating produces a BYE for the non seeded opponent who lost their match' do
    wrestler_two_first_round = @tournament.reload.matches.find { |m| m.bracket_position == 'Quarter' && m.wrestler1.name == 'Test2' }
    wrestler_four_first_round = @tournament.matches.find { |m| m.bracket_position == 'Quarter' && m.wrestler1.name == 'Test4' }
    wrestler_four_first_round_opponent = wrestler_four_first_round.wrestler2.id
    wrestler_two_first_round.wrestler1.destroy

    wrestlers = @tournament.wrestlers
    wrestler_three = wrestlers.find { |w| w.name == 'Test3' }
    wrestler_three.original_seed = 2
    wrestler_three.save
    wrestler_four = wrestlers.find { |w| w.name == 'Test4' }
    wrestler_four.original_seed = 3
    wrestler_four.save
    generate_tournament_matches(@tournament.reload)
    matches = @tournament.reload.matches

    assert(matches.find { |m| m.bracket_position == 'Quarter' && m.w2 == wrestler_four_first_round_opponent }.loser1_name == 'BYE')
  end
  ...
  test 'Changing a wrestler from not seeded to seeded should only change his line and the person who had the line they now get' do
    ...
    original_wrestler4_opponent_name = matches_original.find { |m| m.wrestler1.name == 'Test4' }.wrestler2.name

    wrestler_seven = @tournament.wrestlers.find { |w| w.name == 'Test7' }
    ...
    matches_original.each do |match|
      match_wrestler1_name = @tournament.wrestlers.find { |w| w.id == match.w1 }.name
      match_wrestler2_name = @tournament.wrestlers.find { |w| w.id == match.w2 }.name
      matching_second_match = matches_second.find { |m| m.bracket_position_number == match.bracket_position_number }
      @tournament.wrestlers.find { |w| w.id == matching_second_match.w1 }.name
      match_second_wrestler2_name = @tournament.wrestlers.find { |w| w.id == matching_second_match.w2 }.name
