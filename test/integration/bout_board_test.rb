...
class BoutBoardTest < ActionDispatch::IntegrationTest
  ...
  test 'moving queue2 from mat1 to mat2 shifts queues and unassigns bumped match' do
    ...
    assert_not_includes [mat1_q2.id, mat1_q3.id, mat1_q4.id], mat1.queue4, 'Mat1 queue4 should be a new match'
  ...
  test 'Deleting a mat mid tournament does not delete any matches' do
    ...
    assert_equal 1, @tournament.matches.count { |m| m.bracket_position == '1/2' }, 'Before deleting a mat there should be 1 match for bracket position 1/2'
    assert_equal 1, @tournament.matches.count { |m| m.bracket_position == '3/4' }, 'Before deleting a mat there should be 1 match for bracket position 3/4'
    assert_equal 1, @tournament.matches.count { |m| m.bracket_position == '5/6' }, 'Before deleting a mat there should be 1 match for bracket position 5/6'
    assert_equal 8, @tournament.matches.count { |m|
      m.bracket_position == 'Bracket Round of 16'
    }, 'Before deleting a mat there should be 8 matches for bracket position Bracket Round of 16'
    assert_equal 4, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Round of 8.1'
    }, 'Before deleting a mat there should be 4 matches for bracket position Conso Round of 8.1'
    assert_equal 4, @tournament.matches.count { |m|
      m.bracket_position == 'Quarter'
    }, 'Before deleting a mat there should be 4 matches for bracket position Quarter'
    assert_equal 2, @tournament.matches.count { |m|
      m.bracket_position == 'Semis'
    }, 'Before deleting a mat there should be 2 matches for bracket position Semis'
    assert_equal 4, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Round of 8.2'
    }, 'Before deleting a mat there should be 4 matches for bracket position Conso Round of 8.2'
    assert_equal 2, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Quarter'
    }, 'Before deleting a mat there should be 2 matches for bracket position Conso Quarter'
    assert_equal 2, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Semis'
    }, 'Before deleting a mat there should be 2 matches for bracket position Conso Semis'
    ...
    assert_equal 1, @tournament.matches.count { |m|
      m.bracket_position == '1/2'
    }, 'After deleting a mat there should still be 1 match for bracket position 1/2'
    assert_equal 1, @tournament.matches.count { |m|
      m.bracket_position == '3/4'
    }, 'After deleting a mat there should still be 1 match for bracket position 3/4'
    assert_equal 1, @tournament.matches.count { |m|
      m.bracket_position == '5/6'
    }, 'After deleting a mat there should still be 1 match for bracket position 5/6'
    assert_equal 8, @tournament.matches.count { |m|
      m.bracket_position == 'Bracket Round of 16'
    }, 'After deleting a mat there should still be 8 matches for bracket position Bracket Round of 16'
    assert_equal 4, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Round of 8.1'
    }, 'After deleting a mat there should still be 4 matches for bracket position Conso Round of 8.1'
    assert_equal 4, @tournament.matches.count { |m|
      m.bracket_position == 'Quarter'
    }, 'After deleting a mat there should still be 4 matches for bracket position Quarter'
    assert_equal 2, @tournament.matches.count { |m|
      m.bracket_position == 'Semis'
    }, 'After deleting a mat there should still be 2 matches for bracket position Semis'
    assert_equal 4, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Round of 8.2'
    }, 'After deleting a mat there should still be 4 matches for bracket position Conso Round of 8.2'
    assert_equal 2, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Quarter'
    }, 'After deleting a mat there should still be 2 matches for bracket position Conso Quarter'
    assert_equal 2, @tournament.matches.count { |m|
      m.bracket_position == 'Conso Semis'
    }, 'After deleting a mat there should still be 2 matches for bracket position Conso Semis'
  end
  ...
  test "When matches are generated, they're assigned a mat in round robin fashion" do
    ...
    mat1.reload
    mat2.reload
    matches_ordered_by_bout = @tournament.matches.sort_by(&:bout_number)
