...
class DoubleEliminationSixteenManSixPlacesRunThrough < ActionDispatch::IntegrationTest
  ...
  test '16 man double elimination placing 6 run through' do
    ...
    winner_by_name('Test9', round_of_16.find { |m| m.bracket_position_number == 2 })
    winner_by_name('Test5', round_of_16.find { |m| m.bracket_position_number == 3 })
    winner_by_name('Test4', round_of_16.find { |m| m.bracket_position_number == 4 })
    winner_by_name('Test14', round_of_16.find { |m| m.bracket_position_number == 5 })
    winner_by_name('Test6', round_of_16.find { |m| m.bracket_position_number == 6 })
    winner_by_name('Test10', round_of_16.find { |m| m.bracket_position_number == 7 })

    quarter = matches.select { |m| m.bracket_position == 'Quarter' }.sort_by(&:bracket_position_number)
    ...
    conso_r8_1 = matches.select { |m| m.bracket_position == 'Conso Round of 8.1' }.sort_by(&:bracket_position_number)
    ...
    conso_r8_2 = matches.select { |m| m.bracket_position == 'Conso Round of 8.2' }.sort_by(&:bracket_position_number)
