...
class DoubleEliminationAutoByes < ActionDispatch::IntegrationTest
  ...
  test '8 man double elimination bracket with only 6 guys deletes one guy and auto advances byes and matches with a bye never get assigned to a mat' do
    ...
    @tournament.weights.first.wrestlers.find { |w| w.name == 'Test6' }.destroy

    generate_tournament_matches(@tournament.reload)
    matches = @tournament.matches.reload

    round1 = matches.select { |m| m.round == 1 }
    assert round1.find { |m| m.bracket_position_number == 1 }.wrestler1.name == 'Test1'
    assert round1.find { |m| m.bracket_position_number == 1 }.loser2_name == 'BYE'
    assert round1.find { |m| m.bracket_position_number == 2 }.wrestler1.name == 'Test4'
    assert round1.find { |m| m.bracket_position_number == 2 }.wrestler2.name == 'Test5'
    assert round1.find { |m| m.bracket_position_number == 3 }.wrestler1.name == 'Test3'
    assert round1.find { |m| m.bracket_position_number == 3 }.loser2_name == 'BYE'
    assert round1.find { |m| m.bracket_position_number == 4 }.wrestler1.name == 'Test2'
    assert round1.find { |m| m.bracket_position_number == 4 }.loser2_name == 'BYE'
    winner_by_name('Test4', round1.find { |m| m.bracket_position_number == 2 })
    ...
    conso_semis = matches.select { |m| m.bracket_position == 'Conso Semis' }.sort_by(&:bracket_position_number)
    ...
    test3_wrestler = @tournament.weights.first.wrestlers.find { |w| w.name == 'Test3' }
