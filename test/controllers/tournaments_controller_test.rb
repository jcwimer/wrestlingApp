...
class TournamentsControllerTest < ActionController::TestCase
  ...
  test 'only owner can set user_id when creating a new tournament' do
    ...
    post :create,
         params: {
           tournament: {
             name: 'New Tournament', address: '123 Main St', director: 'Director',
             director_email: 'director@example.com', date: '2024-01-01',
             tournament_type: 'Pool to bracket', is_public: false
           }
         }
  ...
  test 'all match bout numbers render in pool to bracket (two pools to semi) page' do
    ...
    @tournament.matches.where.not(bracket_position: 'Pool').find_each do |match|
      assert_match(/#{match.bout_number}/, response.body, "Bout number #{match.bout_number} is missing from the bracket page")
    end
  ...
  test 'index sorts tournaments by date closest to today' do
    ...
    t_today = Tournament.create!(name: 'Closest Today', address: '123 Test St', director: 'Director', director_email: 'today@example.com', date: today,
                                 tournament_type: 'Pool to bracket', is_public: true)
    t_minus1 = Tournament.create!(name: 'Minus 1', address: '123 Test St', director: 'Director', director_email: 'm1@example.com', date: today - 1,
                                  tournament_type: 'Pool to bracket', is_public: true)
    t_plus1 = Tournament.create!(name: 'Plus 1', address: '123 Test St', director: 'Director', director_email: 'p1@example.com', date: today + 1,
                                 tournament_type: 'Pool to bracket', is_public: true)
    t_plus2 = Tournament.create!(name: 'Plus 2', address: '123 Test St', director: 'Director', director_email: 'p2@example.com', date: today + 2,
                                 tournament_type: 'Pool to bracket', is_public: true)
    t_minus3 = Tournament.create!(name: 'Minus 3', address: '123 Test St', director: 'Director', director_email: 'm3@example.com', date: today - 3,
                                  tournament_type: 'Pool to bracket', is_public: true)
    t_plus10 = Tournament.create!(name: 'Plus 10', address: '123 Test St', director: 'Director', director_email: 'p10@example.com', date: today + 10,
                                  tournament_type: 'Pool to bracket', is_public: true)
    ...
    assert results.index { |r| r.id == t_minus1.id } < results.index { |r|
      r.id == t_plus2.id
    }, 't_minus1 (distance 1) should appear before t_plus2 (distance 2)'
    assert results.index { |r| r.id == t_plus2.id } < results.index { |r|
      r.id == t_minus3.id
    }, 't_plus2 (distance 2) should appear before t_minus3 (distance 3)'
  end
  ...
  test 'index paginates tournaments with page param and exposes total_count' do
    ...
    25.times do |i|
      Tournament.create!(name: "Paginate Test #{i}", address: '1 Paginate Rd', director: 'Dir', director_email: "paginate#{i}@example.com",
                         date: Time.zone.today + i, tournament_type: 'Pool to bracket', is_public: true)
    end
