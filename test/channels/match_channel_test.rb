...
class MatchChannelTest < ActionCable::Channel::TestCase
  ...
  test 'send_stat does not change fragment cache versions' do
    ...
    assert_equal(versions, [@match.tournament, @match.weight, @match.wrestler1&.school, @match.wrestler2&.school].compact.to_h do |record|
      [[record.class.name, record.id], record.reload.cache_key_with_version]
    end)
  end
