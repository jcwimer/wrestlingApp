...
class Tournament < ApplicationRecord
  ...
  def self.search_date_name(pattern)
    if pattern.blank? # blank? covers both nil and empty string
      ...
      where(like_patterns.to_s, *search_variables)
    end
  end

  def days_until_start
    time = (Time.zone.today - date).to_i
    time *= -1 if time.negative?
    time
  end

  def tournament_types
    ['Pool to bracket', 'Modified 16 Man Double Elimination 1-6', 'Modified 16 Man Double Elimination 1-8',
     'Regular Double Elimination 1-6', 'Regular Double Elimination 1-8']
  end

  def number_of_placers
    if tournament_type.include? '1-8'
      8
    elsif tournament_type.include? '1-6'
      6
    end
  end
  ...
  def destroy_with_dependents!
    ...
    %w[schools weights mats wrestlers matches delegates mat_assignment_rules tournament_backups
       tournament_job_statuses].each do |association_name|
  ...
  def remove_school_delegations
    schools.each do |s|
      s.delegates.each(&:destroy)
    end
  end

  def pool_to_bracket_number_of_wrestlers_error
    error_string = ''
    if tournament_type.include? 'Pool to bracket'
      ...
      weights_with_too_many_wrestlers.each do |weight|
        error_string += " The weight class #{weight.max} has more than 24 wrestlers."
      end
      weight_with_too_few_wrestlers.each do |weight|
        error_string += " The weight class #{weight.max} has less than 2 wrestlers."
      end
    end
    error_string
  end
  ...
  def modified_sixteen_man_number_of_wrestlers_error
    error_string = ''
    if tournament_type.include? 'Modified 16 Man Double Elimination'
      ...
      weights_with_too_many_wrestlers.each do |weight|
        error_string += " The weight class #{weight.max} has more than 16 wrestlers."
      end
      weight_with_too_few_wrestlers.each do |weight|
        error_string += " The weight class #{weight.max} has less than 12 wrestlers."
      end
    end
    error_string
  end
  ...
  def double_elim_number_of_wrestlers_error
    error_string = ''
    if (tournament_type == 'Regular Double Elimination 1-6') || (tournament_type == 'Regular Double Elimination 1-8')
      weights_with_too_many_wrestlers = weights.select { |w| w.wrestlers.size > 64 }
      weight_with_too_few_wrestlers = weights.select { |w| w.wrestlers.size < 2 }
      weights_with_too_many_wrestlers.each do |weight|
        error_string += " The weight class #{weight.max} has more than 64 wrestlers."
      end
      weight_with_too_few_wrestlers.each do |weight|
        error_string += " The weight class #{weight.max} has less than 2 wrestlers."
      end
  ...
  def wrestlers_with_higher_seed_than_bracket_size_error
    ...
    weights.each do |weight|
      weight.wrestlers.each do |wrestler|
        next unless !wrestler.original_seed.nil? && wrestler.original_seed > weight.wrestlers.size

        error_string += "Wrestler: #{wrestler.name} has a seed of #{wrestler.original_seed} " \
                        "which is greater than the amount of wrestlers (#{weight.wrestlers.size}) " \
                        "in the weight class #{weight.max}."
      end
    end
    error_string
  end
  ...
  def wrestlers_with_duplicate_original_seed_error
    error_string = ''
    weights.each do |weight|
      weight.wrestlers.reject { |wr| wr.original_seed.nil? }.each do |wrestler|
        if weight.wrestlers.many? { |wr| wr.original_seed == wrestler.original_seed }
  ...
  def wrestlers_with_out_of_order_seed_error
    error_string = ''
    weights.each do |weight|
      original_seeds = weight.wrestlers.filter_map(&:original_seed).sort
  ...
  def match_generation_error
    error_string = ''
    if !pool_to_bracket_number_of_wrestlers_error.empty?
      error_string += pool_to_bracket_number_of_wrestlers_error
    elsif !modified_sixteen_man_number_of_wrestlers_error.empty?
      error_string += modified_sixteen_man_number_of_wrestlers_error
    elsif !double_elim_number_of_wrestlers_error.empty?
      error_string += double_elim_number_of_wrestlers_error
    elsif !wrestlers_with_higher_seed_than_bracket_size_error.empty?
      error_string += wrestlers_with_higher_seed_than_bracket_size_error
    elsif !wrestlers_with_duplicate_original_seed_error.empty?
      error_string += wrestlers_with_duplicate_original_seed_error
    elsif !wrestlers_with_out_of_order_seed_error.empty?
      error_string += wrestlers_with_out_of_order_seed_error
    end
    return "There is a tournament error. #{error_string}" unless error_string.empty?
  ...
  def refill_open_bout_board_queues(invalidate_cached_views: true)
  ...
  alias has_active_jobs? active_jobs?
  ...
  def preload_up_matches_first_rounds(matches)
    weight_ids = matches.filter_map(&:weight_id).uniq
  ...
  def broadcast_bout_board_changes(mat_records)
    cache_values = Rails.cache.read_multi(*mat_records.flat_map do |mat|
