...
module TournamentServices
  class ModifiedSixteenManGenerateLoserNames
    ...
    def assign_loser_names(match_rows = nil)
      rows = match_rows || @tournament.matches.where(tournament_id: @tournament.id).map do |m|
        m.attributes.symbolize_keys
      end
    ...
    def assign_loser_names_in_memory(weight, match_rows)
      ...
      bracket_sixteen_round = rows.select { |r| r[:bracket_position] == 'Bracket Round of 16' }
      consolation_eight_round = rows.select do |r|
        r[:bracket_position] == 'Conso Round of 8'
      end.sort_by { |r| r[:bracket_position_number] }

      consolation_eight_round.each do |row|
        case row[:bracket_position_number]
        when 1
          m1 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 1 }
          m2 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 2 }
          row[:loser1_name] = "Loser of #{m1[:bout_number]}" if m1
          row[:loser2_name] = "Loser of #{m2[:bout_number]}" if m2
        when 2
          m3 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 3 }
          m4 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 4 }
          row[:loser1_name] = "Loser of #{m3[:bout_number]}" if m3
          row[:loser2_name] = "Loser of #{m4[:bout_number]}" if m4
        when 3
          m5 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 5 }
          m6 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 6 }
          row[:loser1_name] = "Loser of #{m5[:bout_number]}" if m5
          row[:loser2_name] = "Loser of #{m6[:bout_number]}" if m6
        when 4
          m7 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 7 }
          m8 = bracket_sixteen_round.find { |m| m[:bracket_position_number] == 8 }
      ...
      conso_quarters = rows.select do |r|
        r[:bracket_position] == 'Conso Quarter'
      end.sort_by { |r| r[:bracket_position_number] }
