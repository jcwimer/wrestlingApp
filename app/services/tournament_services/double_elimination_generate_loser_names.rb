  ...
  class DoubleEliminationGenerateLoserNames
    ...
    def assign_loser_names_in_memory(weight, match_rows)
      ...
      champ_full[0...-1].each_with_index do |champ_info, i|
        ...
        next if map_idx.negative? || map_idx >= conso_rounds.size
      ...
      mappings.each do |map|
        ...
        conso.each do |cm|
          ...
          cm[:loser1_name] = if champ_match1
                               if is_first_feed && single_competitor_match_row?(champ_match1)
                                 'BYE'
                               else
                                 "Loser of #{champ_match1[:bout_number]}"
                               end
                             end
          ...
          if map[:both_wrestlers]
            ...
            cm[:loser2_name] = if champ_match2
                                 if is_first_feed && single_competitor_match_row?(champ_match2)
                                   'BYE'
                                 else
                                   "Loser of #{champ_match2[:bout_number]}"
                                 end
                               end
          end
          idx += 1
      ...
      if bracket_size >= 5 && num_placers >= 6 && weight.wrestlers.size > 4
        conso_semis = rows.select do |r|
          r[:bracket_position] == 'Conso Semis'
      ...
      return unless bracket_size >= 7 && num_placers >= 8 && weight.wrestlers.size > 6

      conso_quarters = rows.select do |r|
        r[:bracket_position] == 'Conso Quarter'
      end.sort_by { |r| r[:bracket_position_number] }
      m78 = rows.find { |r| r[:bracket_position] == '7/8' }
      return unless conso_quarters.size >= 2 && m78
    ...
    def consolation_label(participants, round_index, bracket_size)
      max_round_index_for_bracket = ((2 * (Math.log2(bracket_size).to_i - 1)) - 1)

      if participants == 2 && round_index == max_round_index_for_bracket
        '3/4'
      elsif participants == 4
        round_index.odd? ? 'Conso Quarter' : 'Conso Semis'
      else
        suffix = round_index.odd? ? '.1' : '.2'
