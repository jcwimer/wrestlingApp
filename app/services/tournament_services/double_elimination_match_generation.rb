...
module TournamentServices
  class DoubleEliminationMatchGeneration
    ...
    def define_bracket_matches(bracket_size)
      ...
      return nil unless bracket_size.nobits?(bracket_size - 1)
      ...
      round_one = seeds.each_slice(2).map.with_index do |(s1, s2), _idx|
        a, b = [s1, s2].sort
        {
          seeds: [a, b],
    ...
    def align_rows_to_largest_bracket(rows_by_weight_id)
      largest_weight = generation_weights.max_by(&:calculate_bracket_size)
    ...
    def create_matchup(wrestler1_id, wrestler2_id, bracket_position, bracket_position_number, round, weight)
      {
        w1: wrestler1_id,
        w2: wrestler2_id,
    ...
    def generate_seed_sequence(bracket_size)
      raise ArgumentError, 'Bracket size must be a power of two' unless bracket_size.nobits?(bracket_size - 1)
      return [1, 2] if bracket_size == 2

      half = bracket_size / 2
      prev = generate_seed_sequence(half)
      comp = prev.map { |seed| bracket_size + 1 - seed }
