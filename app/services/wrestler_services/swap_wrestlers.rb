...
module WrestlerServices
  class SwapWrestlers
    ...
    def swap_wrestlers_bracket_lines(wrestler1_id, wrestler2_id)
      ...
      # Swap matches
      swap_wrestler_matches(w1_matches, w1.id, w2.id)
      swap_wrestler_matches(w2_matches, w2.id, w1.id)

      w1.save
      w2.save
    end

    def swap_wrestler_matches(matches_to_swap, from_id, to_id)
      matches_to_swap.each do |m|
        if m.w1 == from_id
          m.w1 = to_id
        elsif m.w2 == from_id
          m.w2 = to_id
        end
        m.winner_id = to_id if m.winner_id == from_id
        m.save
      end
    end
  end
