...
class TournamentBackupsController < ApplicationController
  ...
  before_action :set_tournament_backup, only: %i[show destroy restore]
  ...
  def create
    ...
    redirect_to tournament_tournament_backups_path(@tournament),
                notice: 'Backup was successfully created. It will show up soon, check your background jobs for status.'
  end
  ...
  def restore
    ...
    redirect_to tournament_path(@tournament),
                notice: 'Restore has successfully been submitted, please check your background jobs to see if it has finished.'
  end
  ...
  def set_tournament_backup
    @tournament_backup = @tournament.tournament_backups.find(params.require(:id))
  end
